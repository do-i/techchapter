...
public class LocationPrinter {
  ...
  public LocationPrinter(String fileName) {
    ...
    locations =
        Lists.newArrayListWithCapacity(1000); // tyring to match this to Trip.DEFAULT_DROP_POINTS
  }
  ...
  public void dump(TerminalSignal terminalSignal) {
    ...
    if (!atomicBoolean.getAndSet(true)) {
      Iterable<String> txtLocations =
          locations.stream().map(Object::toString).collect(Collectors.toList());
      try {
